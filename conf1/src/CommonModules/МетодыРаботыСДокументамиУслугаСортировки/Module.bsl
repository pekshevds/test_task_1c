...
//
// Параметры:
//  ЧастьДанныхФайла  - Массив из Строка - строки для загрузки в базу данных 
//
Процедура СоздатьДокументыУслугаСортировки(ЧастьДанныхФайла)Экспорт 
	...
	Пока ЧастьДанныхФайла.Количество() > 0 Цикл 
		...
		ДанныеДляСозданиеДокумента = МетодыРаботыСЗагружаемымФайломИЕгоСодержимымВызовСервера.ПолучитьДанныеДляСозданиеДокумента(
			ТекущаяСтрока);	
...
//
// Параметры:
//  N  - Число - Максимальное количество элементов в выборку
//                 								во временном хранилище
// Возвращаемое значение:
//   Массив из ДокументСсылка.УслугаСортировки  - Массив из N документов
//   
...
//  Тарифы  - Соответствие из ПеречислениеСсылка.КодыТиповОбработки - тарифы на услуги
//  
// Возвращаемое значение:
...
Функция РассчитатьДокументУслугаСортировки(Знач УслугаСортировки, Тарифы)Экспорт
	...
	СведенияПоДнямХраненияГруза = 
		Документы.УслугаСортировки.ПолучитьСведенияПоДнямХраненияГруза(УслугаСортировки.Ссылка);
	...
	Если УслугаСортировки.КодТипаОбработки = Перечисления.КодыТиповОбработки.Хаб Тогда 
		
		ТаблицаТарифов = Тарифы[Перечисления.КодыТиповОбработки.Хаб];
		НайденыеСтроки = ТаблицаТарифов.НайтиСтроки(Новый Структура("ОтпСклМс", УслугаСортировки.ОтпСклМс));
		
		УслугаСортировки.СтоимостьСортировки = ?(НайденыеСтроки.Количество() > 0, 
													НайденыеСтроки[0].Тариф, 
													0);
	Иначе   
...
//  
// Возвращаемое значение:
//   Массив из ДокументСсылка.УслугаСортировки - части выборки документов
//
